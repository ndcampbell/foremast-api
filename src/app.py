...
@app.route("/"+API_VERSION+"/runner/<action>", methods=['POST'])
def runner(action):
...
@app.route("/"+API_VERSION+"/runner/status/<job_key>", methods=['GET'])
...
@app.route("/"+API_VERSION+"/runner/logs/<job_key>", methods=['GET'])
def get_logs(job_key):
    """Returns log as plain text, easiest way to read the log"""
    job = Job.fetch(job_key, connection=conn)
    if job.is_finished:
        logs = job.result
    elif job.is_failed:
...
@app.route("/"+API_VERSION+"/runner/jobs", methods=['GET'])
def get_all_jobs():
